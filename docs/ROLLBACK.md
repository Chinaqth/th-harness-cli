...
`harness update` stages the replacement beside the active Runtime and keeps the previous Runtime until the new files, guidance, and projections are committed. If the transaction fails, it restores the previous Runtime and managed projections automatically.
