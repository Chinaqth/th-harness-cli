...
description: Independently evaluate an assembled Domain Pack for source-traceable content completeness, structural consistency, cross-file references, routing behavior, and automatic activation readiness. Use after all declared artifacts have current passing evaluations or when a final activation-ready, fail, or blocked verdict is required.
---
...
8. Confirm `content_state=content-complete` and `state=activation-ready`, or else return
   `fail`/`blocked`. Preserve organization gaps as non-lifecycle downstream inputs.

## Activation Boundary

`content-complete` means the source-traceable public baseline passes. `activation-ready` means the
Pack may be automatically finalized to `active` without a separate lifecycle approval. Missing
organization or project facts remain explicit and block only dependent task actions or claims.
